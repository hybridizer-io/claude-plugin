...
- **`--flavors` includes a flavor not in `--display-license-details`.** Hard fail at transcode. If the unlicensed flavor is OMP / HIP / AVX and the user is on the free NuGet tool, the fix is the paid standalone (the free tool is CUDA-only), not just a license bump. Run the license check and report what's actually available before recommending action.
